...
class DevFtpBackdown extends Command
{
    protected $signature = 'devftp:backdown';
    ...
    public function handle()
    {
        ...
        $excludePaths[] = base_path('storage' . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'devftp' . DIRECTORY_SEPARATOR . 'backdown');
        ...
        if ($zip->close()) {
            ...
            $this->logInfo(PHP_EOL . 'Backup created successfully: ' . $backupFile);
        } else {
            $this->error(PHP_EOL . 'Failed to finalize the backup.');
            $this->logError(PHP_EOL . 'Failed to finalize the backup.');
        }

        $progressBar->finish();
        $this->logInfo(PHP_EOL . 'Backup process completed.');
        return 0;
    }
